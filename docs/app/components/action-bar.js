...
export default {
  template: `
  <div class="action-bar"><router-link tag="button" :to="{ name: 'editor', params: { slug: $store.getters['dates/backSlug'] }}"><i class="fas fa-angle-left"></i></router-link><div class="btn-group"><button class="icon" @click.prevent="logout"><i class="fas fa-user-alt-slash"></i><label>logout</label></button><button @click.prevent="$store.commit('ui/toggleShy')"><i class="fas fa-eye" v-if="$store.state.ui.isShy"></i><i class="fas fa-eye-slash" v-else></i></button><button class="icon" @click.prevent="save"><i class="fas fa-save"></i><label>Save</label></button><router-link class="icon" tag="button" v-if="!$store.getters['dates/isToday']" :to="{ name: 'editor', params: { slug: $store.getters['dates/todaySlug'] }}"><i class="fas fa-calendar-check"></i><label>Today</label></router-link></div><router-link tag="button" :disabled="$store.getters['dates/isToday']" :to="{ name: 'editor', params: { slug: $store.getters['dates/nextSlug'] }}"><i class="fas fa-angle-right"></i></router-link></div>`,
